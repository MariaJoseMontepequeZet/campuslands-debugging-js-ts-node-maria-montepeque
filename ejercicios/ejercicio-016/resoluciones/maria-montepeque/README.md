...
El `README.md` base describe el objetivo como "corregir penalizaciones en tiempos de
vuelta" (tema carreras, dificultad "Basico retador"), pero el archivo real
`codigo/racing-laps.js` y su test `tests/racing-laps.test.js` no contienen logica de
penalizaciones ni tiempos de vuelta: son el mismo contrato de `calcularResultado` (suma
de puntos) y `ordenarRanking` (orden de mayor a menor) que los ejercicios 001 a 015. Se
corrigio el codigo tal como existe realmente, sin modificar el test ni el archivo base.
...
import { calcularResultado, ordenarRanking } from './ejercicios/ejercicio-016/resoluciones/maria-montepeque/racing-laps.js';
