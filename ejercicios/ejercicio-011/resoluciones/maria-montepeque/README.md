...
El `README.md` base describe el objetivo como "detectar sobrevivientes y zona segura"
(tema battle royale, dificultad "Basico retador"), pero el archivo real
`codigo/battle-zone.js` y su test `tests/battle-zone.test.js` no contienen logica de
sobrevivientes ni zonas: son el mismo contrato de `calcularResultado` (suma de puntos) y
`ordenarRanking` (orden de mayor a menor) que los ejercicios 001 a 010. Se corrigio el
...
import { calcularResultado, ordenarRanking } from './ejercicios/ejercicio-011/resoluciones/maria-montepeque/battle-zone.js';
