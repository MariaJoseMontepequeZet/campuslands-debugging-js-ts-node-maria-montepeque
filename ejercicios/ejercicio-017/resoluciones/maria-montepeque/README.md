...
El `README.md` base describe el objetivo como "simular marcador con reglas de ventaja"
(tema pingpong, dificultad "Basico retador"), pero el archivo real `codigo/pingpong.js`
y su test `tests/pingpong.test.js` no contienen logica de reglas de ventaja ni marcador
de puntos por set: son el mismo contrato de `calcularResultado` (suma de puntos) y
`ordenarRanking` (orden de mayor a menor) que los ejercicios 001 a 016. Se corrigio el
codigo tal como existe realmente, sin modificar el test ni el archivo base.
...
import { calcularResultado, ordenarRanking } from './ejercicios/ejercicio-017/resoluciones/maria-montepeque/pingpong.js';
