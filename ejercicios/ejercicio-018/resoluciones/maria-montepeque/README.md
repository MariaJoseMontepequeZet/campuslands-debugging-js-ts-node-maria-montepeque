...
El `README.md` base describe el objetivo como "parsear formula simple y contar atomos"
(tema formulas quimicas, dificultad "Basico retador"), pero el archivo real
`codigo/formula-parser.js` y su test `tests/formula-parser.test.js` no contienen logica
de parseo de formulas ni conteo de atomos: son el mismo contrato de `calcularResultado`
(suma de puntos) y `ordenarRanking` (orden de mayor a menor) que los ejercicios 001 a
017. Se corrigio el codigo tal como existe realmente, sin modificar el test ni el
archivo base.
...
import { calcularResultado, ordenarRanking } from './ejercicios/ejercicio-018/resoluciones/maria-montepeque/formula-parser.js';
