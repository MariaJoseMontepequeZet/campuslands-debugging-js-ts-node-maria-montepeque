...
El `README.md` base describe el objetivo como "ordenar ranking por victorias y
diferencia" (tema torneo MOBA), pero el archivo real `codigo/moba-ranking.js` y su test
`tests/moba-ranking.test.js` no contienen logica de victorias ni diferencia de goles/kills:
son el mismo contrato de `calcularResultado` (suma de puntos) y `ordenarRanking` (orden de
mayor a menor) que los ejercicios 001, 002 y 003. Se corrigio el codigo tal como existe
realmente, sin modificar el test ni el archivo base.
...
import { calcularResultado, ordenarRanking } from './ejercicios/ejercicio-004/resoluciones/maria-montepeque/moba-ranking.js';
