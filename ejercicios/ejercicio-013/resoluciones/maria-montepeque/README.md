...
El `README.md` base describe el objetivo como "agrupar gastos por ciudad sin
sobrescribir" (tema viajes, dificultad "Basico retador"), pero el archivo real
`codigo/travel-costs.js` y su test `tests/travel-costs.test.js` no contienen logica de
agrupacion por ciudad ni gastos: son el mismo contrato de `calcularResultado` (suma de
puntos) y `ordenarRanking` (orden de mayor a menor) que los ejercicios 001 a 012. Se
...
import { calcularResultado, ordenarRanking } from './ejercicios/ejercicio-013/resoluciones/maria-montepeque/travel-costs.js';
