...
El `README.md` base describe el objetivo como "normalizar capas visibles en un archivo"
(tema dibujo digital, dificultad "Basico retador"), pero el archivo real
`codigo/layers.js` y su test `tests/layers.test.js` no contienen logica de capas ni
visibilidad: son el mismo contrato de `calcularResultado` (suma de puntos) y
`ordenarRanking` (orden de mayor a menor) que los ejercicios 001 a 014. Se corrigio el
...
import { calcularResultado, ordenarRanking } from './ejercicios/ejercicio-015/resoluciones/maria-montepeque/layers.js';
