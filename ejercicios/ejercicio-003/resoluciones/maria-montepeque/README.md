...
El `README.md` base describe el objetivo como "corregir descuento y redondeo de
precios" (tema tienda de skins), pero el archivo real `codigo/skins.js` y su test
`tests/skins.test.js` no contienen logica de descuentos ni precios: son el mismo
contrato de `calcularResultado` (suma de puntos) y `ordenarRanking` (orden de mayor a
menor) que los ejercicios 001 y 002. Se corrigio el codigo tal como existe realmente,
...
import { calcularResultado, ordenarRanking } from './ejercicios/ejercicio-003/resoluciones/maria-montepeque/skins.js';
