...
El `README.md` base describe el objetivo como "evitar mutación inesperada al aplicar
pociones" (tema inventario RPG), pero el archivo real `codigo/inventory.js` y su test
`tests/inventory.test.js` no contienen lógica de inventario ni pociones: son el mismo
contrato de `calcularResultado` (suma de puntos) y `ordenarRanking` (orden de mayor a
menor) que el ejercicio 001. Se corrigió el código tal como existe realmente, sin
modificar el test ni el archivo base, siguiendo la regla de no alterar tests para forzar
que pase una solución distinta al contrato real.
...
import { calcularResultado, ordenarRanking } from './ejercicios/ejercicio-002/resoluciones/maria-montepeque/inventory.js';
