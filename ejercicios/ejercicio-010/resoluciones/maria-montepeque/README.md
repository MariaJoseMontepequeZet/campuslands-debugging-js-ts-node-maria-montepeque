...
El `README.md` base describe el objetivo como "validar estados antes de totalizar
pedidos" (tema pedidos de comida), pero el archivo real `codigo/food-orders.js` y su
test `tests/food-orders.test.js` no contienen logica de estados de pedidos: son el mismo
contrato de `calcularResultado` (suma de puntos) y `ordenarRanking` (orden de mayor a
menor) que los ejercicios 001 a 009. Se corrigio el codigo tal como existe realmente,
...
import { calcularResultado, ordenarRanking } from './ejercicios/ejercicio-010/resoluciones/maria-montepeque/food-orders.js';
