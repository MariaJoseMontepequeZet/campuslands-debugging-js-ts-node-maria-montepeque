...
El `README.md` base describe el objetivo como "calcular tabla con goles a favor/en
contra" (tema futbol sala), pero el archivo real `codigo/futsal-table.js` y su test
`tests/futsal-table.test.js` no contienen logica de goles ni diferencia de gol: son el
mismo contrato de `calcularResultado` (suma de puntos) y `ordenarRanking` (orden de mayor
a menor) que los ejercicios 001 a 007. Se corrigio el codigo tal como existe realmente,
...
import { calcularResultado, ordenarRanking } from './ejercicios/ejercicio-008/resoluciones/maria-montepeque/futsal-table.js';
